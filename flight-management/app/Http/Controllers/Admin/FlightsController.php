    ...
    public function index()
    {
        if (request()->ajax()) {
            $flights = Flight::join('flight_time', 'flight_time.flight_id', '=', 'flights.id')
                    ...
                    ->select('flights.*','flight_time.flight_code as flight_code', 'flight_time.flight_time as flight_time','origin_ap.name as origin_ap_name','destination_ap.name as destination_ap_name')
    ...
    public function delete()
{
    try {
        ...
        if (!$flight) {
            DB::rollBack();
            return $this->errorResponse('Flight not found');
        }
        ...
        DB::rollBack();
        Log::error('Error deleting flight: ' . $exception->getMessage(), [
            'trace' => $exception->getTraceAsString()
        ]);
        return $this->errorResponse('An error occurred while deleting the flight');
    }
}
